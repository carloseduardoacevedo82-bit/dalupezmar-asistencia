...
try {
  ...
  // 8. Sincronizar badges QR para todos los colaboradores respetando su estado (ACTIVE o INACTIVE)
  const employees = db.prepare('SELECT id, employee_code, document_number, status FROM employees').all();
  const insertOrReplaceBadge = db.prepare(`
    INSERT OR REPLACE INTO badges (
      ...
      ?, ?, ?, ?, '2026-01-01', '2028-12-31', ?, 'DALUPEZMAR_OFFICIAL'
    )
  `);
